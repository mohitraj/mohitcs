...
class file_backup():
    ...
    def file_open(self):
        pickle_file = open("fdate.raj", "rb")
        file_dict = cPickle.load(pickle_file)
        pickle_file.close()
        return file_dict

    def strore_files(self):
        pickle_file = open("fdate.raj", "wb")
    ...
    def file_get(self,time_stamp,latest=None,m=0, order= False, number_of_files=0,e_list=None,match=None):
        ...
        for key,values in all_file.items():
            c_date = values[m]
            if c_date>= time_stamp and c_date<= latest:
                d1[key]= c_date
        d2 = OrderedDict(sorted(d1.items(), key=lambda v: v[1], reverse=order))
...
def main():
    ...
    try:
        ...
        if args.cs:
            ...
            print ("Current Version --> %1.1f"%(version1))
